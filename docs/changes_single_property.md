...
## Overview
Updated `test_single_property_cold` to use the `MemoryTracker` probe system for detailed performance analysis.
...
- **Methodology**: Changed from a simple `BenchmarkTimer` wrapper to an explicit loop with `MemoryTracker` context manager, similar to `test_initial_load`.
