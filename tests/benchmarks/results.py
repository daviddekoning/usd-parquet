...
class ProbeResult:
    ...
    elapsed_since_last: float   # seconds since last probe (excludes current probe overhead)
    # Memory
...
class MemoryTracker:
    ...
    def __exit__(self, *args):
        ...
        self.result = MemoryResult(
            current_bytes=max(0, total_delta),
            peak_bytes=max(0, total_delta)
        )
    ...
    def _print_results(self, total_elapsed: float, total_delta: int):
        ...
        print(f"{'='*70}")
        print(f"{'Probe':<25} {'Time (s)':<12} {'Δt (s)':<12} {'Mem (MB)':<12} {'ΔMem (MB)':<12}")
        print(f"{'-'*70}")
        
        for probe in self.probes:
            print(f"{probe.label:<25} "
