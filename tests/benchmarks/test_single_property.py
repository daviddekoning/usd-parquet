...
def _run_single_property_iteration(
    base_scene_path_str, prop_path_str, prim_path, format_name, run_index, result_queue
):
...
def _run_single_property_traversal_iteration(
    base_scene_path_str,
    prop_path_str,
    format_name,
    run_index,
    ...
    try:
        ...
        with PerformanceTracker(
            ...
            if total_visited == 0:
                raise RuntimeError(
                    "No prims were visited during traversal, which is unexpected."
                )
...
class TestSinglePropertyRetrieval:
    ...
    def test_single_property_cold(
        ...
        for i in range(num_runs):
            ...
            try:
                ...
                if probes:
                    ...
                    if i < num_runs - 1:
                        print(
                            f"  Total: {total_time:.4f}s, {total_memory / (1024 * 1024):+.2f} MB\n"
                        )
        ...
        final_memory = MemoryResult(
            current_bytes=int(avg_peak), peak_bytes=int(avg_peak)
        )
...
class TestSinglePropertyTraversal:
    ...
    def test_single_property_traversal(
        ...
        collector.add_result(
            BenchmarkResult(
                ...
                extra={
                    ...
                    "time_per_prim_us": (timing.mean_seconds / last_prim_count)
                    * 1_000_000
        ...
        if last_prim_count != benchmark_scale:
            raise ValueError(
                f"Expected to read property from {benchmark_scale} prims, but got {last_prim_count}"
            )
