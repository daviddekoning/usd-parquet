...
def test_overhead_exclusion():
    ...
    for probe in tracker.probes:
        print(f"{probe.label:<20} {probe.elapsed_since_start:>18.4f}s "
    ...
    for i, (probe, expected) in enumerate(zip(tracker.probes, expected_times)):
        ...
        print(f"  Probe {i+1} elapsed_since_start: {actual:.4f}s "
    ...
    for i, probe in enumerate(tracker.probes):
        ...
        print(f"  Probe {i+1} delta: {delta:.4f}s "
              f"(expected ~0.1s) {status}")
    ...
    print(f"Average overhead per probe: {tracker._total_probe_overhead / len(tracker.probes) * 1000:.2f}ms")
