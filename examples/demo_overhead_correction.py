...
def demo_with_explanation():
    ...
    print("\nMeasured Results:")
    print(
        f"{'Step':<10} {'Cumulative Time':>16} {'Step Time':>12} {'Expected':>12} {'Status':>8}"
    )
    print("-" * 70)
    ...
    for i, (probe, expected) in enumerate(zip(tracker.probes, expected_cumulative), 1):
        ...
        print(
            f"{probe.label:<10} {probe.elapsed_since_start:>14.4f}s "
    ...
    print(
        f"Average per probe: {tracker._total_probe_overhead / len(tracker.probes) * 1000:.3f}ms"
    )
